...
class MapSettings:
    def __init__(self):
        ...
        self.zoom_level = 13  # начальный масштаб
        self.map_type = "map"
...
def adjust_zoom(settings, direction):
    if direction == "up" and settings.zoom_level < 19:
        settings.zoom_level += 1
    elif direction == "down" and settings.zoom_level > 1:
        settings.zoom_level -= 1
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_PAGEUP:
                    adjust_zoom(map_settings, "up")
                elif event.key == pygame.K_PAGEDOWN:
                    adjust_zoom(map_settings, "down")
